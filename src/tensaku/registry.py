...
@role: Tensaku 内の軽量レジストリ（関数/クラス/ファクトリの名前→オブジェクト解決）
@inputs:
  - register(name: str)(obj): デコレータで登録
  - get(name: str): 登録オブジェクトを取得（無ければ lazy import を試行）
  - create(name: str, **kw): 取得オブジェクトが callable の場合に呼び出して生成
  - list_names(): 現在登録されている名前一覧
@notes:
  - **lazy import + active bootstrap**:
      未登録 name を要求されたら tensaku.confidence / tensaku.trustscore を import。
      import 副作用で登録されない場合でも、ここで **能動的に register** して埋める。
  - これにより、プロセスを跨いだ「list_names() が空」の問題を解消する。
"""
